  ...
  if ($result > 0) {
   ...
   echo json_encode($arrayName,JSON_UNESCAPED_UNICODE);
     ...
     $id_e = mb_strtoupper(md5($email.date('d-m-Y H:m:s').$nombres));
      $query_insert=mysqli_query($conection,"INSERT INTO usuarios(nombres,email,password,codigo_registro,pais,ciudad,provincia,ip,url_img_upload,id_e,user_in,terminos)
                                                    VALUES('$nombres','$email','$password1','$codigo_registro','$pais','$ciudad','$provincia','$direccion_ip','$url','$id_e','$user_in','Aceptada') ");
