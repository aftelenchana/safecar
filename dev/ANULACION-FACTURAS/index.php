<?php
// ======================================================
// API Rebotadora → SRI Login/Anulación (Render)
// Entrada:  { "KEY":"...", "usuario":"...", "clave":"...", "clave_acceso":"...", "identificacion":"...", "correo":"..." }
// Salida:   JSON del endpoint remoto + tracking + cuota + cobro real (producto 13606)
...
$clave_acceso   = strtoupper(trim($in['clave_acceso'] ?? ''));
$identificacion = preg_replace('/\D+/', '', (string)($in['identificacion'] ?? ''));
$correo         = trim($in['correo'] ?? '');

// Validaciones mínimas (ajusta a tu gusto)
if ($KEY === '')                                    { echo json_encode(['error' => 'KEY vacía.']); exit(); }
if ($usuario === '')                                { echo json_encode(['error' => 'usuario requerido.']); exit(); }
if ($clave === '')                                  { echo json_encode(['error' => 'clave requerida.']); exit(); }
if ($clave_acceso === '' || strlen($clave_acceso) < 20) { echo json_encode(['error' => 'clave_acceso inválida.']); exit(); }
if ($identificacion === '' || strlen($identificacion) < 10) { echo json_encode(['error' => 'identificacion inválida.']); exit(); }
if ($correo === '' || !filter_var($correo, FILTER_VALIDATE_EMAIL)) { echo json_encode(['error' => 'correo inválido.']); exit(); }
...
// Config general
$query_config = mysqli_query($conection, "SELECT * FROM configuraciones LIMIT 1");
$result_config = $query_config ? mysqli_fetch_array($query_config) : null;
$cuota_sms_api = $result_config['cuota_sms_api'] ?? 0;

// Autenticación de KEY
...
$idProductoApi = 13654; // ajusta si corresponde
...
$FREE_MONTHLY    = (int)$cuota_sms_api; // máximo gratis por mes
...
// para tracking: usa clave_acceso si viene; si no, identificación; si no, usuario
$busqueda_val   = $clave_acceso !== '' ? $clave_acceso : ($identificacion !== '' ? $identificacion : $usuario);
$busqEsc        = mysqli_real_escape_string($conection, $busqueda_val);

// ========= Rebotar al endpoint de Render (POST JSON) =========
$endpoint = "https://anulacionfactura-yc4t.onrender.com/sri/login";
...
function post_json_with_retry($url, $jsonBody, $maxSeconds = 240, $connectTO = 30, $retries = 3) {
  ...
  while ($attempt < $retries) {
    ...
    curl_setopt_array($ch, [
      ...
      CURLOPT_MAXREDIRS         => 3,
      // CURLOPT_SSL_VERIFYPEER => false, // sólo si tu CA falla (no recomendado)
    ]);
  ...
  try {
    ...
    mysqli_query($conection, "
      INSERT INTO historial_bancario(
        ...
        '{$iduser}', '{$costo_consulta}', 'API Consulta', 'API-SRI-LOGIN', '0', 'Ninguno'
      )
    ");
