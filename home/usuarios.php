  ...
  <div class="container-fluid py-4">
    <div class="table-container">
      <div class="d-flex justify-content-between align-items-center table-header">
        <h2 class="mb-0"><i class="fas fa-piggy-bank me-2"></i> Gestión de Usuarios</h2>
...
$(document).ready(function() {
  ...
  const tabla = $('#usuariosTable').DataTable({
    ajax: {
      ...
      type: 'POST',
      data: { action: 'consultar_datos_usuarios' }
    },
    columns: [
      { data: 'nombres' },
      { data: 'identificacion' },
      ...
      { data: 'tipo_cliente' },
      {
        ...
        render: function(data) {
          ...
          if (data == '0') return '<span class="badge bg-danger">Inactivo</span>';
  ...
  $('#usuariosTable tbody').on('click', '.btn-ver', function() {
    const id = $(this).data('id');
    $('#modalEmpresaTitle').text('Ver Usuario');
    $('#modalEmpresaBody').html(`<p>Cargando datos del usuario ID: ${id}</p>`);
    modal.show();
  });

  $('#usuariosTable tbody').on('click', '.btn-editar', function() {
    const id = $(this).data('id');
    $('#modalEmpresaTitle').text('Editar Usuario');
    $('#modalEmpresaBody').html(`<p>Formulario para editar usuario ID: ${id}</p>`);
    modal.show();
  });

  $('#usuariosTable tbody').on('click', '.btn-eliminar', function() {
    const id = $(this).data('id');
    $('#modalEmpresaTitle').text('Eliminar Usuario');
