...
include "../../coneccion.php";
 ...
 if ($_POST['action'] == 'agregar_cuenta_bancaria') {
     ...
     $nombreBanco   =  mysqli_real_escape_string($conection,$_POST['nombreBanco']);
     $tipo_cuenta         =  mysqli_real_escape_string($conection,$_POST['tipo_cuenta']);
     $numero_cuenta       =  mysqli_real_escape_string($conection,$_POST['numero_cuenta']);
     $moneda              =  mysqli_real_escape_string($conection,$_POST['moneda']);
     $saldo_inicial       =  mysqli_real_escape_string($conection,$_POST['saldo_inicial']);

     $limiteDiario        =  mysqli_real_escape_string($conection,$_POST['limiteDiario']);
     $fechaApertura       =  mysqli_real_escape_string($conection,$_POST['fechaApertura']);
     $fechaCierre         =  mysqli_real_escape_string($conection,$_POST['fechaCierre']);
     $interes             =  mysqli_real_escape_string($conection,$_POST['interes']);

     $permiteCheques      =  mysqli_real_escape_string($conection,$_POST['permiteCheques']);
     $serieCheques        =  mysqli_real_escape_string($conection,$_POST['serieCheques']);
     $cantidadCheques     =  mysqli_real_escape_string($conection,$_POST['cantidadCheques']);
     $notas               =  mysqli_real_escape_string($conection,$_POST['notas']);
     $cuentaActiva        =  mysqli_real_escape_string($conection,$_POST['cuentaActiva']);

     $query_insert=mysqli_query($conection,"INSERT INTO cuentas_bancarias_factu  (iduser, nombreBanco ,tipo_cuenta,numero_cuenta,moneda,saldo_inicial,empresa,limiteDiario,fechaApertura,fechaCierre,interes,permiteCheques,serieCheques,cantidadCheques,notas,cuentaActiva)
                                   VALUES('$iduser','$nombreBanco','$tipo_cuenta','$numero_cuenta','$moneda','$saldo_inicial','$empresa','$limiteDiario','$fechaApertura','$fechaCierre','$interes','$permiteCheques','$serieCheques','$cantidadCheques','$notas','$cuentaActiva') ");
...
if ($_POST['action'] == 'consultar_datos_cuentas_bancarias') {
  ...
  mysqli_query($conection,"SET lc_time_names = 'es_ES'");
     $sql = "
         ...
         WHERE cuentas_bancarias_factu.iduser = '$iduser'
             AND cuentas_bancarias_factu.estatus = '1'
     ";


          $sql .= " ORDER BY cuentas_bancarias_factu.fecha DESC  ";
