  ...
  <div class="container-fluid py-4">
    <div class="table-container">
      <div class="d-flex justify-content-between align-items-center table-header">
        <h2 class="mb-0"><i class="fas fa-building me-2"></i> Gestión de Empresas</h2>
...
$(document).ready(function() {
  ...
  const tabla = $('#empresasTable').DataTable({
    ajax: {
      ...
      type: 'POST',
      data: { action: 'consultar_datos_empresas' }
    },
    columns: [
      { data: 'identificacion' },
      { data: 'razon_social' },
      ...
      { data: 'email' },
      {
        ...
        render: function(data) {
          ...
          if (data == '0') return '<span class="badge bg-secondary">Inactivo</span>';
  ...
  $('#empresasTable tbody').on('click', '.btn-ver', function() {
    ...
    $('#modalEmpresaTitle').text('Ver Empresa');
    $('#modalEmpresaBody').html(`<p>Cargando datos de la empresa ID: ${id}</p>`);
    modal.show();
  });

  $('#empresasTable tbody').on('click', '.btn-editar', function() {
    const id = $(this).data('id');
    $('#modalEmpresaTitle').text('Editar Empresa');
    $('#modalEmpresaBody').html(`<p>Formulario para editar la empresa ID: ${id}</p>`);
    modal.show();
  });

  $('#empresasTable tbody').on('click', '.btn-eliminar', function() {
    const id = $(this).data('id');
    $('#modalEmpresaTitle').text('Eliminar Empresa');
