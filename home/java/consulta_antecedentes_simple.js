(() => {
  ...
  // Puede venir desde data-endpoint con http://localhost/dev/antecedentes/simple
  const ENDPOINT       = WRAP.dataset?.endpoint || 'https://api.guibis.com/dev/antecedentes/simple';
  ...
  function renderSummary(payload){
    ...
    // Si realmente no hay nada que mostrar, escondemos el panel
  ...
  btnEnviar.addEventListener('click', async () => {
    ...
    // JSON ENVIADO (antecedentes/simple usa el mismo contrato)
    const requestBody = {
  ...
  btnDownloadReq.addEventListener('click', () => {
    try {
      ...
      const filename = `requestANTECEDENTES_${appSlug}_${paramSlug}_${ts()}.json`;
  ...
  btnDownload.addEventListener('click', () => {
    try {
      ...
      const appSlug = slugify(CURRENT_APP_NAME || 'app');
      const paramSlug = slugify(CURRENT_PARAM || 'param');
      const filename = `responseANTECEDENTES_${appSlug}_${paramSlug}_${ts()}.json`;
