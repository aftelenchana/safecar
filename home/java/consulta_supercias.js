(() => {
  ...
  // Puede venir desde data-endpoint con http://localhost/dev/supercias/
  const ENDPOINT       = WRAP.dataset?.endpoint || 'https://api.guibis.com/dev/supercias/';
  ...
  btnEnviar.addEventListener('click', async () => {
    ...
    // === JSON ENVIADO (Supercias usa mismo contrato) ===
    const requestBody = {
  ...
  btnDownloadReq.addEventListener('click', () => {
    try {
      ...
      const filename = `requestSUPERCIAS_${appSlug}_${paramSlug}_${ts()}.json`;
  ...
  btnDownload.addEventListener('click', () => {
    try {
      ...
      const appSlug = slugify(CURRENT_APP_NAME || 'app');
      const paramSlug = slugify(CURRENT_PARAM || 'param');
      const filename = `responseSUPERCIAS_${appSlug}_${paramSlug}_${ts()}.json`;
