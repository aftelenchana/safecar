(() => {
  ...
  const ENDPOINT     = WRAP?.dataset.endpoint || 'https://api.guibis.com/dev/ruc-cedula/';
  ...
  let CURRENT_PARAM = "";      // para filename
  ...
  // ===== Resumen robusto (usa quota y/o billing) =====
  function renderSummary(payload){
    ...
    // Toma costo: primero quota.costo_consulta si existe, si no billing.costo_total, si no costo_unitario
  ...
  // ===== Eventos =====
  if (!WRAP) return; // por si no existe el wrapper
  ...
  btnDownloadReq.addEventListener('click', () => {
    try {
      ...
      const filename = `request_${appSlug}_${paramSlug}_${ts()}.json`;
  ...
  btnDownload.addEventListener('click', () => {
    try {
      ...
      const appSlug = slugify(CURRENT_APP_NAME || 'app');
      const paramSlug = slugify(CURRENT_PARAM || 'param');
      const filename = `response_${appSlug}_${paramSlug}_${ts()}.json`;
