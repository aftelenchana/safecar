(() => {
  ...
  const ENDPOINT     = 'java/nombres_apellidos.php';
  ...
  // Archivo guardado / PDF
  const savedJsonArea = document.getElementById('savedJsonArea');
  const savedJsonLink = document.getElementById('savedJsonLink');
  const savedJsonHint = document.getElementById('savedJsonHint');
  ...
  function clearPanels(){
    ...
    if (savedJsonArea) {
      savedJsonArea.classList.add('d-none');
      savedJsonLink?.removeAttribute('href');
      if (savedJsonLink) savedJsonLink.textContent = '';
      if (savedJsonHint) savedJsonHint.textContent = '';
    }
  ...
  function renderSummary(payload){
    ...
    if (gratis !== '—' || usadas !== '—' || maxm !== '—' ||
        esPago || costoNum || saldoInicioNum || saldoRestNum || rangoMes) {
      ...
      summary.innerHTML = `
        <div class="row g-3">
          ...
          <div class="col-md-6">
            <div class="border rounded p-3 bg-white">
              <div class="small text-muted mb-1"><i class="fa-solid fa-credit-card me-1"></i>Financiero</div>
  ...
  // ===== Convierte el botón PDF en enlace =====
  function upgradePdfButtonToLink() {
    if (!btnGenerarPDF) return null;
    if (btnGenerarPDF.tagName.toLowerCase() === 'a') return btnGenerarPDF;
    const a = document.createElement('a');
    a.id = btnGenerarPDF.id;
    a.className = btnGenerarPDF.className;
    a.innerHTML = btnGenerarPDF.innerHTML;
    a.href = '#';
    a.target = '_blank';
    a.rel = 'noopener';
    btnGenerarPDF.replaceWith(a);
    return a;
  }
  pdfLink = upgradePdfButtonToLink();
  ...
  parametro.addEventListener('input', () => {
    const pos = parametro.selectionStart;
    parametro.value = (parametro.value || '').toUpperCase();
    try { parametro.setSelectionRange(pos, pos); } catch {}
  });
  ...
  btnEnviar.addEventListener('click', async () => {
    ...
    CURRENT_PARAM = paramUpper;

    const requestBody = { parametro_busqueda: paramUpper, KEY: CURRENT_KEY };
    ...
    try{
      ...
      if(!res.ok || data?.error){
        ...
        if (saved?.ok && saved?.url && saved?.filename && savedJsonArea && savedJsonLink) {
          savedJsonLink.setAttribute('href', saved.url);
          savedJsonLink.textContent = saved.filename;
          savedJsonArea.classList.remove('d-none');
          if (savedJsonHint) savedJsonHint.textContent = 'Archivo JSON guardado en el servidor.';

          // ✅ Generar enlace PDF
          if (!pdfLink || pdfLink.tagName.toLowerCase() !== 'a') {
            pdfLink = upgradePdfButtonToLink();
          }
          if (pdfLink) {
            const par  = encodeURIComponent(CURRENT_PARAM || '');
            const file = encodeURIComponent(saved.filename);
            pdfLink.href   = `pdf/consultas/reporte_nombres.php?f=${file}&param=${par}`;
        ...
        showAlert('success', '<i class="fa-solid fa-circle-check me-2"></i>Consulta realizada y guardada con éxito.');
      }
    }catch(err){
  ...
  // Copiar / Descargar JSON (igual)
  btnCopyReq?.addEventListener('click', async ()=>{ try{ await navigator.clipboard.writeText(jsonReqRaw.textContent);
    btnCopyReq.innerHTML = '<i class="fa-solid fa-check me-1"></i>Copiado'; setTimeout(()=> btnCopyReq.innerHTML = '<i class="fa-solid fa-copy me-1"></i>Copiar', 1200);}catch(e){}});

  btnDownloadReq?.addEventListener('click', ()=>{try{const c=jsonReqRaw.textContent||'{}';let p=c;try{p=JSON.stringify(JSON.parse(c),null,2);}catch{};const b=slugify(CURRENT_PARAM||'param');const f=`request_${b}_${ts()}.json`;const blob=new Blob([p],{type:"application/json"});const u=URL.createObjectURL(blob);const x=document.createElement("a");x.href=u;x.download=f;document.body.appendChild(x);x.click();x.remove();URL.revokeObjectURL(u);}catch(e){showAlert('danger',`<i class="fa-solid fa-circle-xmark me-2"></i>Error al descargar: ${e.message}`);}});
  ...
  btnDownload?.addEventListener('click', ()=>{try{const c=jsonRaw.textContent||'{}';let p=c;try{p=JSON.stringify(JSON.parse(c),null,2);}catch{};const b=slugify(CURRENT_PARAM||'param');const f=`response_${b}_${ts()}.json`;const blob=new Blob([p],{type:"application/json"});const u=URL.createObjectURL(blob);const x=document.createElement("a");x.href=u;x.download=f;document.body.appendChild(x);x.click();x.remove();URL.revokeObjectURL(u);}catch(e){showAlert('danger',`<i class="fa-solid fa-circle-xmark me-2"></i>Error al descargar: ${e.message}`);}});
})();
