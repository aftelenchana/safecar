...
$iduser= $_SESSION['id'];
 ...
 $query_asiento_contable = mysqli_query($conection, "SELECT * FROM contabilidad_asientos_contables
    WHERE contabilidad_asientos_contables.iduser ='$iduser'  AND contabilidad_asientos_contables.estatus = '1' AND contabilidad_asientos_contables.id = '$codigo' ");
 $data_asiento_contable  = mysqli_fetch_array($query_asiento_contable);

 $referencia = $data_asiento_contable['referencia'];



 $protocol = empty($_SERVER['HTTPS']) ? 'http://' : 'https://';$domain = $_SERVER['HTTP_HOST'];$url2 = $protocol . $domain;

 $gg = "$url2/home/pdf/contable/asiento_contable_out.php?codigo=$codigo&iduser=$iduser";


 $html=file_get_contents_curl("$url2/home/pdf/contable/asiento_contable_out.php?codigo=$codigo&iduser=$iduser");
...
$pdf->stream('Asiento_Contable-'.$referencia.'.pdf');
