...
// Ejemplo de uso:
$DIRECCION = $camposAdicionales['DIRECCION'];
$correo   = $camposAdicionales['correo'];
$telefono= $camposAdicionales['telefono'];
$No_COMPR      = $camposAdicionales['No.COMPR'];
    ...
    <div class="parte_inermedia">
      <table>
        <tbody>
          ...
          <tr>
            <td> <span class="td_bld" >Dirección:</span> <?php echo $DIRECCION ?></td>
          </tr>
          <tr>
            <td> <span class="td_bld" >Celular:</span> <?php echo $telefono ?> </td>
          </tr>
          <tr>
    ...
    <div class="parte_productos">
      <div class="">
        <table class="parte_productos_solo_productos">
            ...
            <tbody>
              <?php
              // Información general de la retención
              $periodoFiscal = $acceso_retencion->infoCompRetencion->periodoFiscal;

              $totalValorRetenido = 0; // Inicializa la variable para sumar los valores retenidos
              // Iterar sobre cada impuesto en el XML
              foreach ($acceso_retencion->impuestos->impuesto as $impuesto) {
                  $codigo = (string)$impuesto->codigo;
                  $codigoRetencion = (string)$impuesto->codigoRetencion;
                  $baseImponible = (float)$impuesto->baseImponible;
                  $porcentajeRetener = (float)$impuesto->porcentajeRetener;
                  $valorRetenido = (float)$impuesto->valorRetenido;
                  $numDocSustento = (string)$impuesto->numDocSustento;
                  $fechaEmision = (string)$impuesto->fechaEmisionDocSustento;

                  // Sumar el valor retenido al total
                  $totalValorRetenido += $valorRetenido;

                  // Convertir el código en texto legible
                  if ($codigo == '1') {
                      $codigo = 'RENTA';
                  } elseif ($codigo == '2') {
                      $codigo = 'IVA';
              ...
              <tr>
                ...
                <td class="td_productos"><?php echo htmlspecialchars($numDocSustento); ?></td>
                <td class="td_productos"><?php echo htmlspecialchars($fechaEmision); ?></td>
                <td class="td_productos"><?php echo htmlspecialchars($periodoFiscal); ?></td>
                <td class="td_productos"><?php echo number_format($baseImponible, 2); ?></td>
                <td class="td_productos"><?php echo htmlspecialchars($codigo); ?></td>
                <td class="td_productos"><?php echo htmlspecialchars($codigoRetencion); ?></td>
                <td class="td_productos"><?php echo number_format($porcentajeRetener, 2); ?>%</td>
                <td class="td_productos"><?php echo number_format($valorRetenido, 2); ?></td>
              </tr>
              <?php
              }
              ?>
              <!-- Total de valores retenidos -->
              <tr>
                <td colspan="8" class="td_productos" style="text-align: right; font-weight: bold;">Total Valor Retenido:</td>
                <td class="td_productos" style="font-weight: bold;"><?php echo number_format($totalValorRetenido, 2); ?></td>
              </tr>
            </tbody>
          </table>
      </div>
        ...
        <div class="informacion_ghd">
          <table class="tabla_ghdd">
            <tbody>
              ...
              <tr class="tr_info_gjd" >
                <td> <span class="td_bld" >Email Sujeto a Retener:</span> <span><?php echo $correo ?></td>
              </tr>
            </tbody>
          </table>
